...
LLM_EMBEDDING_PROVIDER=openai  # Options: openai, gemini, openrouter, custom

# Custom OpenAI-compatible embedding endpoint (for Ollama, vLLM, etc.)
LLM_CUSTOM_EMBEDDING_API_KEY=ollama
LLM_CUSTOM_EMBEDDING_BASE_URL=http://localhost:11434/v1
LLM_CUSTOM_EMBEDDING_MODEL=nomic-embed-text
...
# Embedding dimensions. Must match the active embedding model.
...
6. **Vector Store Issues**
   ...
   - If you change dimensions in `pgvector`, clear or re-embed stored vectors first
