...
def fixed_embedding_vector() -> list[float]:
    ...
    Returns a config-sized vector with predictable values to avoid
    network calls to external embedding services during testing.
    """
    # Create a deterministic config-sized embedding vector
    # Using a simple pattern that's easy to verify in tests
    return [0.1 * (i % 10) for i in range(settings.VECTOR_STORE.DIMENSIONS)]
